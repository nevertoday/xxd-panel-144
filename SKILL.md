---
name: xxd-panel-144
description: "Create XXD Panel 144 raster artwork from photographs using fine vector-like linework, one geometric colour anchor, white knockout and boundary-breaking overlaps, a tiny stamp-like focal core and enormous whitespace. Use when the user invokes xxd-panel-144 or requests this style. Supports individual images, isolated directory batches, strict 50:50 comparisons, design-only outputs, and wallpapers."
---
...
- The canonical presentation follows the source’s final amendment: a 7:6 left–right canvas with reality left and the source-brief transformation right, exactly 50:50. This overrides the source’s initial top–bottom 3:4 wording. Explicit user-selected modes and sizes take precedence over this default.
...
For each output, concatenate the complete verbatim Chinese source brief, then a short delivery preamble, exactly one selected mode contract, exactly one text contract, and only the user's explicit non-style requirements. Runtime additions may change delivery variables only. The last selected runtime mode and size override all earlier source layout terms, including its final side-by-side amendment. In `left-right`, map upper/lower to left/right; in explicitly selected `top-bottom`, map left/right back to upper/lower. Preserve the aesthetic and strict equal halves. In `design-only` and `wallpaper-pack`, the design occupies the entire canvas and the source is not displayed. Never add an outer palette, title, slogan, or aesthetic theory.
...
Write final PNGs directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-144/` or the explicit output root. Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.
...
- `references/xxd-panel-144-prompt.zh-CN.md` and `.en.md` — delivery adapter notes
